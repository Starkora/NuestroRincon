...
export default function DiaryPage() {
  ...
  const handleUpdate = async () => {
    if (!selectedEntry || !editEntry.title.trim() || !editEntry.content.trim()) return
  ...
  const cancelEdit = () => {
    ...
    if (selectedEntry) {
      setEditEntry({
        ...
        is_shared: selectedEntry.is_shared
      })
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-100 to-blue-100 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Entry Detail Modal */}
        {selectedEntry && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-6 z-50" onClick={() => { setSelectedEntry(null); setIsEditing(false); }}>
            <div className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="p-8">
                {!isEditing ? (
                  ...
                  <>
                    <div className="flex items-start justify-between mb-6">
                      <h2 className="text-2xl font-bold text-purple-900">Editar Entrada</h2>
                      <button
                        onClick={cancelEdit}
                        className="text-gray-400 hover:text-gray-600 transition"
                      >
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full p-4 rounded-xl border-2 border-purple-200 focus:border-purple-500 focus:outline-none text-gray-900"
                          maxLength={100}
                        />
                      </div>
                        ...
                        <div className="grid grid-cols-4 md:grid-cols-8 gap-2">
                          {moods.map((mood) => (
                            <button
                              key={mood.emoji}
                              type="button"
                              onClick={() => setEditEntry({ ...editEntry, mood: mood.emoji })}
                              className={`p-3 rounded-xl border-2 transition-all ${
                                editEntry.mood === mood.emoji
                                  ? 'border-purple-500 bg-purple-50 scale-110'
                                  : 'border-gray-200 hover:border-purple-300'
                              }`}
                              title={mood.label}
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-purple-900 mb-2">Contenido</label>
                        <textarea
                          value={editEntry.content}
                          onChange={(e) => setEditEntry({ ...editEntry, content: e.target.value })}
                          className="w-full p-4 rounded-xl border-2 border-purple-200 focus:border-purple-500 focus:outline-none resize-none text-gray-900"
                          rows={8}
                          maxLength={2000}
                        />
                        <div className="text-sm text-gray-500 mt-1">{editEntry.content.length}/2000 caracteres</div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 bg-purple-50 p-4 rounded-xl">
                        <input
                          type="checkbox"
                          id="edit_is_shared"
                          checked={editEntry.is_shared}
                          onChange={(e) => setEditEntry({ ...editEntry, is_shared: e.target.checked })}
                          className="w-5 h-5 rounded border-purple-300 text-purple-600 focus:ring-purple-500"
                        />
                        <label htmlFor="edit_is_shared" className="text-sm text-purple-900">
                          <span className="font-semibold">Compartir con mi pareja</span>
                          <span className="block text-xs text-purple-600">Si no lo marcas, solo tú podrás verlo</span>
                        </label>
                      </div>
                      ...
                      <div className="flex gap-4 pt-6 border-t">
                        <button
                          ...
                          className="flex-1 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white py-3 rounded-xl font-semibold transition-all"
                        >
                          Guardar Cambios
                        </button>
                        <button
